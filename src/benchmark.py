...
from yahoo_data_parser import parse_line, YahooDataParser
...
def benchmark_lin_ucb(benchmarker, alpha: float):
    ...
    initial_arms_records = initial_arms.article_data[
        ~initial_arms.article_data["article_feature_1"].isna()
    ][benchmarker.article_features].to_records(index=False)
    ...
    for _ in tqdm(
        range(benchmarker.init_record_index, benchmarker.final_record_index - 1),
        total=benchmarker.final_record_index - benchmarker.init_record_index - 1,
    ):
        ...
        article_data = new_data.article_data[
            ~new_data.article_data["article_feature_1"].isna()
        ...
        user_record = (
            user_data.drop(
                labels=["timestamp", "displayed_article_id", "user_click"], axis=1
    ...
    benchmarker.lin_ucb_disjoint_ctr = {
...
class YahooDataBenchmark:
    ...
    def __init__(self, path: str, n: float = np.inf):
        ...
        self.article_features = ["article_id"] + [
            f"article_feature_{x}" for x in range(1, 7)
        ]
    ...
    def random_policy_ctr(self):
        ...
        counter = 0
        for i in tqdm(
            range(self.init_record_index, self.final_record_index, 1),
            total=self.final_record_index - self.init_record_index,
        ):
            counter += 1
...
if __name__ == "__main__":
    ...
    benchmarker = YahooDataBenchmark(path=data_path, n=int(1e3))
    ...
    # Plot it
    ctr_df = pd.DataFrame(vals)
    ctr_plot = (
        pn.ggplot(ctr_df, pn.aes(x="alpha", y="ctr"))
        + pn.geom_line()
        + pn.geom_hline(yintercept=random_ctr, linetype="dotted", color="red")
        + pn.theme_538()
    )
    pn.ggsave(
        ctr_plot,
        filename=base_dir / "figures/benchmark_ctr.png",
        dpi=300,
        height=6,
        width=8,
    )
