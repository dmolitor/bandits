...
class GaussianBandit:
    ...
    def regret_ub(self) -> float:
        ...
        rhs = self.delta + (4 / self.delta) * (
            1 + np.max([0, np.log(self.n * self.delta**2 / 4)])
        )
...
if __name__ == "__main__":
    ...
    regret_df = pd.melt(
        (pd.DataFrame(regret_and_ub).assign(delta=deltas)),
        id_vars=["delta"],
        var_name="type",
        value_name="value",
    ).assign(
        type=lambda x: x["type"].map(
            lambda s: (
                "Cumulative Regret"
                if s == "regret_cumulative"
                else ("Regret Upper Bound" if s == "regret_ub" else s)
            )
        )
    )
