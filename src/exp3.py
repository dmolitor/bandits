...
class AdversarialExp3:
    ...
    def fit(self) -> None:
        ...
        for t in range(self.n):
            p_t_denominator = (
                np.sum([np.exp(self.eta * last(x)) for x in self.s_hat])
            )
            p_t = [
                np.exp(self.eta * last(x)) / p_t_denominator
                for x in self.s_hat
            ]
            ...
            for idx in range(self.k):
                ...
                s_ti = last(self.s_hat[idx]) + 1 - (int(a_t == idx) * (1 - reward))/p_t[idx]
                self.s_hat[idx].append(s_ti)
    
    def regret_cumulative(self) -> List[float]:
        cum_regret = [
            (self.n * reward) - np.sum(self.realizations)
...
if __name__ == "__main__":
    ...
    regret_plot = (
        ...
        + pn.labs(
            x="eta",
            y="Expected regret",
            title="Regret as a function of learning rate"
        )
        + pn.theme_538()
        + pn.theme(
